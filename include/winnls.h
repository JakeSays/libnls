...
/*  ---- libnls preamble --------------------------------------------------
 *
 *  This header was vendored from Wine.  Wine's winnls.h relies on a
 *  handful of macros and the `STRICT` define that are normally set up
 *  by Wine's other Win32 headers (windef.h, winbase.h, etc.) before
 *  inclusion.  libnls ships winnls.h as a stand-alone public surface,
 *  so we seed those prereqs here under `#ifndef` guards.
 *
 *  Consumers responsible for the basic Win32 type vocabulary (DWORD,
 *  LCID, WCHAR, LPARAM, GUID, SYSTEMTIME, ...) need to have them in
 *  scope before including this file — typically by including
 *  <windows.h> first, the same as on Windows.  libnls's internal TUs
 *  use src/nls_compat.h for that.
 */
...
/*  libnls is unconditionally a wide implementation (16-bit WCHAR via
 *  -fshort-wchar).  Every `DECL_WINELIB_TYPE_AW(X)` aliases X to XW. */
#ifndef DECL_WINELIB_TYPE_AW
#define WINELIB_NAME_AW( name )     name##W
#define DECL_WINELIB_TYPE_AW( type ) typedef type##W type;
#endif

/*  Pointer-flavoured types referenced by the declarations below but
 *  not always carried in a consumer's <windows.h>.  Each is a thin
 *  typedef alias; the C++ language allows the same typedef to appear
 *  more than once with the same definition, so the guards just keep
 *  the diagnostics quiet when a consumer already has them. */
#ifndef PULONG_DEFINED
#define PULONG_DEFINED
typedef ULONG*           PULONG;
typedef ULONGLONG*       PULONGLONG;
#endif
#ifndef PZZWSTR_DEFINED
#define PZZWSTR_DEFINED
typedef WCHAR*           PZZWSTR;
typedef const WCHAR*     PCZZWSTR;
#endif
#ifndef FARPROC_DEFINED
